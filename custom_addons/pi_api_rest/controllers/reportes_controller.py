...
class ReportesController(http.Controller):
    ...
    @http.route('/api/reportes/<int:reporte_id>', type='json', auth='none', methods=['PUT'])
    @jwt_required
    def actualizar_reporte(self, reporte_id, **kwargs):
        """Actualiza el estado de un reporte (solo para administradores idealmente)"""
        try:
            usuario_data = request.usuario_actual
            ...
            # Solo el reportador puede actualizar su reporte (o admin en el futuro)
            if reporte.reportado_por_id.id != usuario.id:
                return APIUtils.error_response('No tienes permisos para actualizar este reporte', 403)
            
            vals = {}
            if 'motivo' in kwargs:
                vals['motivo'] = kwargs['motivo']
            if 'estado' in kwargs:
                vals['estado'] = kwargs['estado']
            
            if vals:
                reporte.sudo().write(vals)
    ...
    @http.route('/api/reportes/<int:reporte_id>', type='json', auth='none', methods=['DELETE'])
    @jwt_required
    def eliminar_reporte(self, reporte_id, **kwargs):
        """Elimina un reporte"""
        try:
            usuario_data = request.usuario_actual
            ...
            # Solo el reportador puede eliminar su reporte
            if reporte.reportado_por_id.id != usuario.id:
                return APIUtils.error_response('No tienes permisos para eliminar este reporte', 403)
